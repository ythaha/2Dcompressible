...
struct CONTROL {
    ...
    float Ra_temp,Ra_comp,Ra_comp_a; 
...
#include "advection.h"
   ...
   struct Segment segment;
    ...
    float *MASS[MAX_LEVELS],*Mass,*MassForHF;               /* lumped mass matrix (diagonal elements) for p-g solver etc. */  
    float *tw;
    float *stress;
    float *XP[4],*X[4],*XX[MAX_LEVELS][4],*Interp[MAX_LEVELS][4];
    float *ZZ;
    float *T,*C,*CE,*IHeat,*buoyancy,*rho_r;		
    float *Tdot,*Cdot;		
//    float *heating_visc,*heating_adi,*heating_latent;		
    float *heating_latent;
    double *strain_rate_2;	//square of strain rate at each node
    struct HEATINGNODE *heating_visc,*heating_adi;
    float *heatflux,*heatflux_adv;		
    float *Fas670,*Fas410;		
