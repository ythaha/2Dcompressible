...
//WWWWWWWWWWWWWWWWWWW
//fprintf(stderr,"%d %f level %d\n",j,E->BI[level][j],level);
	    }

      if (E->control.verbose)   {
        fprintf(E->fp,"output stiffness matrix!!!\n");
...
//for compressible cases, construct c matrix, c = \frac{d rho_r}{dr} / rho_r * u_3
void construct_elt_cs(E)
     struct All_variables *E;
{ int el,lev,a;
  int i;
  void get_elt_c();
...
//      printf("lev = %d, el = %d",lev,el); 
//      for(i=0;i<=23;i++)
//	      printf(" %f",E->elt_c[lev][el].c[i][0]);
//      printf("\n");
      }
