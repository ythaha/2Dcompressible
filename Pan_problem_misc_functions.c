...
{ 
    ...
    void phase_change();
    float *buoyancy_T, *buoyancy_P;
    buoyancy_T = (float *) malloc((E->mesh.nno+1)*sizeof(float));
    buoyancy_P = (float *) malloc((E->mesh.nno+1)*sizeof(float));
   ...
   if(E->control.compressibility !=0 && E->control.dyn_p_in_buoyancy) {
   	dp_to_nodes(E,E->P,E->NP,E->mesh.levmax);
	for(i=1;i<=E->mesh.nno;i++)   {
		//???? How to determine the sign? Answer: should be the same as E->T
		buoyancy_P[i] += (-E->control.compressibility)*E->NP[i];
	}
//	remove_horiz_ave(E,buoyancy_P,H,0);
   }
...
/*  for constant g in the mantle */
       ...
       buoyancy_T[i] += E->control.Ra_temp * E->T[i] 
                      * (slope*E->X[2][i] + 1.0);
	   else {
		   buoyancy_T[i] += E->control.Ra_temp * E->T[i] * exp((1-E->X[2][i])*E->control.compressibility)
	                         * (slope*E->X[2][i] + 1.0);
	   }
	  
   }

   //remove_horiz_ave(E,buoyancy_T,H,0);

   for(i=1;i<=E->mesh.nno;i++)
	   E->buoyancy[i] += buoyancy_P[i] + buoyancy_T[i] - E->control.Ra_comp*E->C[i];

   remove_horiz_ave(E,E->buoyancy,H,0);
   

   free ((void *) H);
   free ((void *) buoyancy_T);
   free ((void *) buoyancy_P);
   return;
}
